...
/* Lines 4-17 for if program is compiled on windows OS */
#ifdef _WIN32
...
/* Enumeration of possible l(isp) val(ue) types */
enum {LVAL_ERR, LVAL_NUM, LVAL_SYM, LVAL_STR,
      LVAL_SEXPR, LVAL_FUN, LVAL_QEXPR};
...
/* Builds new environment for function */
...
/* Copies lval in order to retain data when putting things into */
/* and taking things out of the environment */
...
/* Increases count of expression by one */
/* Reallocates the amount of space required by v->cell */
/* Sets final value of the list to the lval* x passed in */
...
/* Forward declare to resolve dependancy */
...
/* Iterates over items in both lists and deletes/frees them */
void lenv_del(lenv* e){
  for(int i = 0; i < e->count; i++){
    free(e->syms[i]);
    lval_del(e->vals[i]);
...
lval* lenv_get(lenv* e, lval* k){
  ...
  for (int i = 0; i < e->count; i++){
    ...
    /* If so, return a copy of the value */
...
/* Defines variables in the global environment */
...
#define LASSERT_TYPE(func, args, index, expect) \
  LASSERT(args, args->cell[index]->type == expect, \
    "Function '%s' passed incorrect type for argument %i. Received: %s, Expected: %s.", \
    func, index, ltype_name(args->cell[index]->type), ltype_name(expect))

#define LASSERT_NUM(func, args, num) \
  LASSERT(args, args->count == num, \
    "Function '%s' passed incorrect number of arguments. Received: %i, Expected: %i.", \
    func, args->count, num)
...
lval* builtin_var(lenv* e, lval* a, char* func){
  ...
  LASSERT(a, (syms->count == a->count - 1),
    ...
    "Received: %i, Expected: %i.", func, syms->count, a->count - 1);
...
lval* builtin_op(lenv* e, lval* a, char* op){
	...
	while (a->count > 0){
		...
		/* Delete element that we are now finished with */
		lval_del(y);
	}
...
/* Converts S-expression to Q-expression */
lval* builtin_list(lenv* e, lval* a){
  a->type = LVAL_QEXPR;
  return a;
}

/* Converts Q-expression to S-expression and evaluates */
  ...
  while (a->count){
    ...
    if (f->formals->count == 0){
      ...
      return lval_err(
        ...
        "Received: %i, Expected: %i", given, total);
    }
...
lval* lval_read(mpc_ast_t* t){
  ...
  if (strstr(t->tag, "string")){
	...
	/* If root (>), qexpr, or sexpr then create an empty list */
	lval* x = NULL;
	if (strcmp(t->tag, ">") == 0){
		x = lval_sexpr();
	}
